...
public class DryingRackBlockEntity extends BlockEntity {
    ...
    public DryingRackBlockEntity(BlockPos pPos, BlockState pBlockState) {
        super(FUBlockEntityTypes.DRYING_RACK.get(), pPos, pBlockState);
    ...
    public boolean addItem(ItemStack itemStackIn, DryingRecipe recipe, int slot) {
    ...
    public Optional<DryingRecipe> getMatchingRecipe(Container recipeWrapper, int slot) {
        if (this.level == null) {
            return Optional.empty();
        } else {
            if (this.lastRecipeIDs[slot] != null) {
                Recipe<Container> recipe = ((RecipeManagerAccessor) this.level.getRecipeManager()).getRecipeMap(FURecipeTypes.DRYING.get()).get(this.lastRecipeIDs[slot]);
                if (recipe instanceof DryingRecipe && recipe.matches(recipeWrapper, this.level)) {
                    return Optional.of((DryingRecipe) recipe);
                }
            }

            return this.level.getRecipeManager().getRecipeFor(FURecipeTypes.DRYING.get(), recipeWrapper, this.level);
        }
    }
